        ...
        /* Return the paginated links for use in themes. */
            ...
            <nav class="navigation paging-navigation" role="navigation">
                <div class="pagination loop-pagination">
                    <?php $prev_posts = get_previous_posts_link('<span class="page_prev"><i class="tficon-chevron-left-alt2"></i></span>'); ?>
                    <?php if ($prev_posts != '') { echo $prev_posts;} else { echo '<a class="page_prev" href="javascript:void(0);"><i class="tficon-chevron-left-alt2"></i></a>'; }?>
                    <div class="page-number">
                        <span><?php echo $current; ?></span> <?php _e('of','tfuse'); ?> <span><?php echo $max_num_pages; ?></span>
                    </div>
                    <?php $next_posts = get_next_posts_link('<span class="page_next"><i class="tficon-chevron-right-alt2"></i></span>'); ?>
                    <?php if ($next_posts != '') { echo $next_posts;} else { echo '<a class="page_next" href="javascript:void(0);"><i class="tficon-chevron-right-alt2"></i></a>'; } ?>
                    <?php //echo $page_links; ?>
                </div>
            </nav>
            <?php
      ...
      <?php }
        ...
        { ?>
            <a href="<?php echo home_url(); ?>">
                <strong><?php echo tfuse_options('logo_text','Joannah Cruz'); ?></strong>
                <span><?php echo tfuse_options('logo_subtitle_text','fitness  trainer'); ?></span>
            </a>
            ...
            if ($count > 0)
            {
                ...
                {                      
                    ...
                    if($template_parent != $term->parent)
                        ...
                        {
                            $term_list_view_all .= '<li class="categories-item active"><a href="'.get_bloginfo('url').'/?cat=' .$term->term_id.  '">' . $term->name . '</a></li>';
                        }
                        elseif($template_parent==$term->term_id)
                        {

                            $term_list_view_all .= '<li class="categories-item"><a href="'.get_bloginfo('url').'/?cat=' .$term->term_id.  '" >' . $term->name . '</a></li>';
                        }
                        ...
                        if( $template_parent==$term->parent)
                        {
                            if($term->slug == $template_slug)
                            {		
                                $term_list .= '<li class="categories-item active"><a href="'.get_bloginfo('url').'/?cat=' .$term->term_id.  '" >' . $term->name . '</a></li>';
                            }
                            else
                            {

                                $term_list .= '<li class="categories-item"><a href="'.get_bloginfo('url').'/?cat=' .$term->term_id.  '" >' . $term->name . '</a></li>';
                            }
                        }
                }
        ...
        foreach ($posts as $k => $post) : $k++;
            ...
            if($slider['design'] == 'stories'){
                ...
                if($slider_image_resize)
                {
                    ...
                    $tfuse_image = '<img src="'.wp_get_attachment_url( get_post_thumbnail_id($post->ID, 'post-thumbnails')).'" alt=""/>';
                
                $terms = wp_get_post_terms( $post->ID, 'stories');
                ...
                $slides[$k]['slide_category'] = !empty($terms) ? $terms[0]->name : '';
            }
            if($slider['design'] == 'workouts'){                
                $tfuse_image = tfuse_page_options('header_image','',$post->ID);
                
                if(empty($tfuse_image)) continue;
                
                $type = tfuse_page_options('workout_type','',$post->ID);
                
                $title = get_the_title($post->ID);
                $slides[$k]['slide_title'] = $title;
                $slides[$k]['slide_src'] = $tfuse_image;
                $slides[$k]['slide_left_column'] = tfuse_page_options('left_column','',$post->ID);
                $slides[$k]['slide_right_column'] = tfuse_page_options('right_column','',$post->ID);
                $slides[$k]['slide_url'] = get_permalink();
                $slides[$k]['slide_id'] = $post->ID;
            }
            elseif($slider['design'] == 'workout')
            {
                $slides['slide_title'] = get_the_title($post->ID);
                $slides['slide_url'] = get_permalink();
            }
        endforeach;
		wp_reset_postdata();
...
if( !function_exists('tfuse_theme_styles') )
{
    function tfuse_theme_styles($values = array(),$options = array())
    {
        ...
        $h_font = $options[TF_THEME_PREFIX . '_subtitles_font'];
        ...
        if(!empty($body_font))
        {
            switch ($body_font) {
                ...
                case 'gafata':
                    ...
                    break;
                default:
                    $font = 'http://fonts.googleapis.com/css?family=Montserrat:400,700';
        ...
        if(!empty($h_font))
        {
            switch ($h_font) {
                ...
                case 'roboto':
                    ...
                    break;
                default:
                    $hand_font = 'http://fonts.googleapis.com/css?family=Montserrat:400,700';
                    $h_family ='"Montserrat", sans-serif';
                    break;
            }
        }
        ...
        if(!empty($titles_font))
        {
            switch ($titles_font) {
                ...
                case 'roboto':
                    ...
                    break;
                default:
                    $t_font = 'http://fonts.googleapis.com/css?family=Montserrat:400,700';
                    $t_family ='"Montserrat", sans-serif';
                    break;
            }
        }
        ...
        if(!empty($hand_font)){
            if(!in_array( $hand_font, $fonts))
            {
                $output .= '
                        @import url('.$hand_font.');';
                $fonts[$hand_font] = $hand_font;
            }
            
            $output .= '
                    @subtitles-font:  '.$h_family.';';
        }
        else{
            $output .= '
                    @subtitles-font:  '.$h_family.';';
        ...
        $text_transform = $options[TF_THEME_PREFIX . '_caps_sub'];
        
        if($text_transform == 'false')
            $output .= '
                    @text-transform-subtitles: lowercase;';
...
//        
        if(!empty($text_color))
        {
            $output .= ' @base-color5: '.$text_color.';
                        @bmi-color: '.$text_color.';
                        @quote-text: '.$text_color.';
                    ';
        }
        ...
        if(!empty($menu_text_color))
        {
            $output .= ' @menu-text-color: '.$menu_text_color.';
                        @logo-color: '.$menu_text_color.';
                        @main-menu-font-color: '.$menu_text_color.';
                    ';
        }
        ...
        {
            $output .= '
                    @base-color7: '.$sec_bg_color.';';
        }
           
        if(!empty($primary_color))
        {
            $output .= '
                    @base-color1: '.$primary_color.';';
            $output .= '
                    @reset-like-button: '.$primary_color.';
                    @subtitles-color: '.$primary_color.';
                    @headings-hover: '.$primary_color.';
        ...
        if(!empty($menu_color))
            $output .= '
                    @main-menu-background: '.$menu_color.';';

        
        if(!empty($heading_color))
        {
            $output .= '
                   @header-color: '.$heading_color.';';
            $output .= '
                    @calendar-slider-title: '.$heading_color.';
                    @calendar-item-title: '.$heading_color.';
                    @section-title-before: '.$heading_color.';
                    @mega-nav-title: '.$heading_color.';
                   ';
        }
...
if(!function_exists('tfuse_sticky_menu'))
{
    function tfuse_sticky_menu(){
        global $TFUSE;
        
        $sticky = tfuse_options('sticky');
        
        if($TFUSE->request->isset_GET('menu') || $sticky)
        {
            if($sticky)
                echo 'sticky-menu';
            elseif($TFUSE->request->GET('menu') == 'sticky')
                echo 'sticky-menu';
        }
    }
}
        ...
        if(!empty($tags)){ 
            ...
            if(!empty($query->posts)){ ?>
                <section class="post-similar clearfix">
                    <h5 class="section-title"><?php _e('You may also like these posts','tfuse'); ?></h5>
                    <?php foreach($query->posts as $item){
                        $image = wp_get_attachment_url( get_post_thumbnail_id($item->ID, 'post-thumbnails'));
                        if(!empty($image))
                            echo '<a href="'.get_permalink($item->ID).'"><img src="'.TF_GET_IMAGE::get_src_link($image, 371, 234).'" /><span>'.strip_tags($item->post_title).'</span></a>';
                    } ?>
                </section>
            <?php }
        }
...
{
    function tfuse_get_workout_trainings(){
        $trainings = tfuse_page_options('content_weeks');
        
        $weeks = 0; $new_trainings = array();
        if(!empty($trainings))
        {
            $count = 0;
            foreach ($trainings as $training) {
                if($training['tab_repeat'] == 'no')
                {
                    $weeks += 1;
                    $trainings_numb = 1;
                }
                ...
                {
                    $weeks += ($training['tab_repeat']+1);
                    $trainings_numb = (int)($training['tab_repeat']+1);
                }
                
                for($t = 0 ; $t < $trainings_numb ; $t++)
                    $new_trainings[] = $training;
                $count++;
            }
        }
        
        $info['weeks'] = $weeks;
        $info['trainings'] = $new_trainings;
...
if ( !function_exists('tfuse_pass_ajax_info')):
    ...
    function tfuse_pass_ajax_info() {
        ...
        {  
            if($posttype == 'advice')
            { 
                $term = get_term_by('slug', get_query_var('term'), 'nutrition'); $type = 'nutrition';
                
                if(empty($term))
                {
                    $term = get_term_by('slug', get_query_var('term'), 'tags_advice'); $type = 'tags_advice';
                }
            }
            elseif($posttype == 'story')
            { 
                $term = get_term_by('slug', get_query_var('term'), 'stories'); $type = 'stories'; 

                if(empty($term))
                {
                    $term = get_term_by('slug', get_query_var('term'), 'tags'); $type = 'tags';
                }
                                }
            elseif($posttype == 'exercise')
            { 
                $term = get_term_by('slug', get_query_var('term'), 'exercises'); $type = 'exercises';
                
                if(empty($term))
                {
                    $term = get_term_by('slug', get_query_var('term'), 'tags_exercises'); $type = 'tags_exercises';
                }
            }
            elseif($posttype == 'service')
            { 
                $term = get_term_by('slug', get_query_var('term'), 'services'); $type = 'services';
                
                if(empty($term))
                {
                    $term = get_term_by('slug', get_query_var('term'), 'tags_service'); $type = 'tags_service';
                }
            }
...
if ( !function_exists('tfuse_get_cat_id')):
	...
	$ID = $posttype = '';
        ...
        if(is_tax())
        {
            if($posttype == 'advice')
            {
                $term = get_term_by('slug', get_query_var('term'), 'nutrition');
                
                if(empty($term))
                     $term = get_term_by('slug', get_query_var('term'), 'tags_advice');
            }
            elseif($posttype == 'story')
            {
                $term = get_term_by('slug', get_query_var('term'), 'stories');
                
                if(empty($term))
                     $term = get_term_by('slug', get_query_var('term'), 'tags');
            }
            elseif($posttype == 'exercise')
            {
                $term = get_term_by('slug', get_query_var('term'), 'exercises');
                
                if(empty($term))
                     $term = get_term_by('slug', get_query_var('term'), 'tags_exercises');
            }
            elseif($posttype == 'service')
            {
                $term = get_term_by('slug', get_query_var('term'), 'services');
                
                if(empty($term))
                     $term = get_term_by('slug', get_query_var('term'), 'tags_service');
            }
