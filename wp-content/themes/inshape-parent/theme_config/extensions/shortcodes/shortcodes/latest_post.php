...
function tfuse_shortcode_latest_post($atts, $content = null)
{
    ...
    if(!empty($recent_post))
    {
        foreach ($recent_post as $post) {
			...
			$time = strtotime(get_the_time( get_option('date_format'), $post['ID'] ));
            
            
        $out .= '<article class="post">
                    <h2 class="entry-title"><a href="'.get_permalink($post['ID']).'">'.get_the_title($post['ID']).'</a></h2>
                    <div class="entry-meta">
                        <time class="entry-date" datetime="'.date('Y-m-d',$time).'T'.date('g:i:s',$time).'">'.get_the_time( get_option('date_format'), $post['ID'] ).'</time>
                    ...
                    $out .='</p></div>
                    <div class="entry-meta">
                        <a href="'.get_permalink($post['ID']).'" class="btn btn-small btn-transparent btn-full"><span>'.__('find out more','tfuse').'</span></a>
                    </div>
                </article>';
        }
    }
