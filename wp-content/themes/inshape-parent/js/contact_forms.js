...
var _theme_index=tf_script.TF_THEME_PREFIX;
        ...
        function showErrorMessage(textMessage){
            _messages = _form.closest('.concatformform').find('.submit_message');
...
function check_fields() {
    ...
    _required_inputs.each(function () {
        ...
        if (jQuery.trim(jQuery(this).val()) == '') {
            _return_val = false;
            console.log(jQuery(this).css('border', '3px solid red'));
        } else {
	...
	if( _captcha_resp && _return_val )
    {
        ...
        _form.closest('.concatformform').find('#header_message').hide();
