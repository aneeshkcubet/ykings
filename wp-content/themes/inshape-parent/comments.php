...
if(is_user_logged_in()){
    $comment_field = ' <p class="comment-form-comment logged">
                            <label for="comment">'. __( 'Message', 'tfuse' ) .'<span class="required">*</span></label>
    ...
    'fields' => apply_filters( 'comment_form_default_fields', array(
        'comment_field' => $textarea_field,
            'author' =>'<p class="comment-form-author">
                            ...
                            <input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) .'" placeholder="' . __( 'Name ', 'tfuse' ) . '" ' . $aria_req . '>
                        </p>',

            'email' =>'<p class="comment-form-email">
                            <label for="email">' . __( 'YOUR EMAIL ADDRESS ', 'tfuse' ) . '<span class="required">*</span></label>
                            <input id="email" name="email" type="email" value="' . esc_attr(  $commenter['comment_author_email'] ) .'" placeholder="' . __( 'Email', 'tfuse' ) . '" ' . $aria_req . '>
                            <span class="optional">(' . __( 'this will not be shared', 'tfuse' ) . ')</span>
                        </p>',
            'url' =>
            '<p class="comment-form-url">
                <label for="url">'.__('YOUR WEBSITE','tfuse').'</label>
                <input id="url" name="url" type="url" value="' . esc_attr( $commenter['comment_author_url'] ) .'" placeholder="'.__('Website','tfuse').'">
