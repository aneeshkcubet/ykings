...
class Recent_Posts_Widget extends WP_Widget {	
	...
	function __construct() {
    	$widget_ops = array(
			...
			'description' => esc_html__('Shows recent posts / custom post types....','cactusthemes')
		);
    	parent::__construct('advanced-recent-posts', esc_html__('GP-Advanced Recent Posts','cactusthemes'), $widget_ops);
	}
		...
		if($the_query->have_posts()):
			...
			while($the_query->have_posts()): $the_query->the_post();$i++;
				...
				if($date == 'on'):
					$date_html .= '                           
						<!-- Begin Date & Time --> 
						<span class="rt-date-posted"><span class="icon-calendar '.(ot_get_option('font_awesome')=='4'?'fa fa-calendar':'').'">'.date_i18n(get_option('date_format')  ,get_the_time('U')).'</span></span> 
						<!-- End Date & Time -->
					';
				endif;
				$comment_num_html = '';
				if($comment_num == 'on'):
					$comment_num_html .= '
						...
						<a href="#" class="rt-comment-text"> <span class="icon-comment '.(ot_get_option('font_awesome')=='4'?'fa fa-comment':'').'">'.get_comments_number().' '.esc_html__('Comments','cactusthemes').'</span> </a> 
						</div>                                        
						<!-- End Comments -->
					';
				endif;	
