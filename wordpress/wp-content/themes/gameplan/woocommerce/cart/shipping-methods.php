...
if ( $available_methods ) {
	...
	foreach ( $available_methods as $method ) {
		...
		if ( $method->cost > 0 ) {
			...
			$method->full_label .= ' (' . esc_html__( 'Free', 'woocommerce' ) . ')';
		}
...
// No shipping methods are available
	...
	if ( ! $woocommerce->customer->get_shipping_country() || ! $woocommerce->customer->get_shipping_state() || ! $woocommerce->customer->get_shipping_postcode() ) {
		...
		echo '<p>' . esc_html__( 'Please fill in your details to see available shipping methods.', 'woocommerce' ) . '</p>';
		...
		echo apply_filters( 'woocommerce_no_shipping_available_html',
			'<p>' .
			sprintf( esc_html__( 'Sorry, it seems that there are no available shipping methods for your location (%s).', 'woocommerce' ) . ' ' . esc_html__( 'If you require assistance or wish to make alternate arrangements please contact us.', 'woocommerce' ), $customer_location ) .
			'</p>'
		);
