...
function parse_carousel_func($atts, $content){
	...
	$html = '
		<div class="posts-carousel responsive carousel_recent '.$animation_class.'">
	';
	$id = rand();
	if($the_query->have_posts()){		
		...
		global $post;
			$meta_html = ($showmetadata) ? '
				<div class="datetime"><span class="icon-calendar fa fa-calendar"></span>'.get_the_date().'</div>
				' : '';

		 if ('open' == $post->comment_status){
			$metacom_html = ($showmetadata) ? '
				<div class="comment"><span class="icon-comment fa fa-comment"></span>'.get_comments_number().' '.__('Comments','cactusthemes').'</div>' : '';
		}
		...
		$html .= '
			...
			<div class="slides-control">
				...
				<div class="control-a">
					<a href="javascript:void(0)" id="posts-prev-'.$id.'" class="icon-sign-blank fa fa-square">
						<span class="icon-caret-left fa fa-caret-left"><!----></span>
					</a>
					<a href="javascript:void(0)" id="posts-next-'.$id.'" class="icon-sign-blank fa fa-square">
...
add_shortcode( 'recent-post', 'parse_carousel_func' );

function recent_post_js(){
	global $recent_post_js;
	if($recent_post_js) foreach($recent_post_js as $arecent_post_js){echo $arecent_post_js;}
}
add_action('wp_footer', 'recent_post_js', 100);

/* Register shortcode with Visual Composer */
add_action( 'after_setup_theme', 'reg_recent_post' );
function reg_recent_post(){
	if(function_exists('wpb_map')){

	wpb_map( array(
		"name"		=> __("Recent post", "js_composer"),
		"base"		=> "recent-post",
