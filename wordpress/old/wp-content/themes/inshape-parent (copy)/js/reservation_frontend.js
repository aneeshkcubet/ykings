...
function showErrorMessage(textMessage){
    var $=jQuery;
    _message= _form.closest('.contactformform').find('#form_messages').html('<h2>'+textMessage+'</h2>').addClass('error_submiting_form').show();
    _form.hide();
}
...
function showSuccessMessage(textMessage){
    var $=jQuery;
    _message=  _form.closest('.contactformform').find('#form_messages').html('<h2>'+textMessage+'</h2>').addClass('success_submited_form').show();
    _form.hide();
...
function check_fields() {
	...
	if (_captcha.length > 0) {
        ...
        $.ajax({
            ...
            success:function (response, textStatus, XMLHttpRequest) {
                if (!response) {
                    _captcha_input.css('border', '3px solid red');
                    _captcha_resp = false;
                }
    ...
    if (_form.find('.'+_theme_index + '_email').length>0) {
		...
		if( _captcha_resp && _return_val )
        {
            ...
            _form.closest('.contactformform').find('#header_message').hide();
